...
* **Traced the entire -$223.32 unhedged drag to three of 23 markets.** `lol-maz-mg1` alone accounts for -$190.26 (70%); the other 18 markets hold no inventory. All three losers show `up_bid = 0.999` or a completely unreadable book with our shares on the losing side — binary settlement risk on a decided event, not diffuse adverse selection.
* **Established that the headline unhedged number is a marking convention.** `fleet_dash.py:574` marks a naked leg at the current bid and writes $0 when the book will not load, so the same position spans -$345.33 (all naked legs lose) to +$168.59 (all win). Risk triggers must key off `naked_cost` and position age, not `unhedged_float`, which moves when a book stops loading rather than when anything economic happens.
* **Isolated the 40–60c entry band as the loss source.** Drift by fill price: 0–20c +$23.37, 20–40c -$4.17, 40–60c **-$122.00** (n=19, mean -10.68c/share), 60–80c +$72.64, 80–100c +$0.22. Every other band is net positive.
* **Measured drift decay by holding horizon.** -1.21c at 5 min (n=52), -5.86c at 1 h (n=27), -12.83c at 6 h (n=5). Monotonic, but the long horizon is too thin to calibrate a time cutoff on. The distribution is fat-tailed: median -1.3c against a -5.24c mean, with only 9 of 52 fills below -20c.
* **Found an instrumentation bug in the quality gate.** `markout.py:_stats_from_rows` averages fills unweighted, so the smallest fills (the worst outliers) drag the reading to -5.24c where the size-weighted figure is -0.87c. All 23 markets are consequently marked `WIDENED`, including 18 holding no inventory — fleet-wide income suppressed by three sick markets.
* **Showed skew never engaged on the worst position.** `lol-maz-mg1` opened 98 naked UP shares, then added 135 more to the same side 30 minutes later (233 total, zero DOWN ever bought). At 98 shares, `skew_full_shares = 240` with `max_skew = 0.015` yields 0.6c — under one tick — and `max_naked_shares = 360` was never approached. Eight of 18 filled markets never paired a share; two-sided markets pair at 89–92%.
* **Cleared execution quality from the investigation.** 57/57 fills are `reason='tape'` (venue-volume confirmed), 0 crossed, $0 taker fees, +2.4c mean spread capture. Every dollar of loss is created after the fill, in inventory management.
* **Published the forensic report.** `research/unhedged_pnl_analysis.html` — 9 charts and 13 sections, self-contained, regenerable from `run/fleet.db` plus `run/fleet_state.json`, using P&L definitions copied verbatim from `server/fleet_dash.py`.
